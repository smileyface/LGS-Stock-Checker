...
class SunValleyGaming(Store):
    def __init__(self):
        super().__init__(
            name="Sun Valley Gaming",
            slug="sun_valley_gaming",
            homepage="https://www.sunvalleygaming.com/",
            search_url="https://www.sunvalleygaming.com/products/search",
    ...
    def _scrape_listings(self, card_name: str) -> List[Dict[str, Any]]:
        ...
        if product_listings:
            for product in product_listings:
                name_element = product.select_one("h4.name, h5.name")
                scraped_card_name = name_element.get("title", "").strip() if name_element else ""

                if not scraped_card_name or card_name.lower() not in scraped_card_name.lower():
    ...
    def _get_product_listings(self, soup: BeautifulSoup) -> List[Any]:
        # This site uses both li.product and li.product.enable-msrp
    ...
    def _parse_variants(self, product: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        variants = []
        # This theme has duplicate variant sections. We target the one inside the 'meta' div.
        meta_section = product.select_one("div.meta")
        if not meta_section:
            return []

        for variant_row in meta_section.select("div.variant-row.in-stock"):
            try:
                description_element = variant_row.select_one(".variant-description")
                price_element = variant_row.select_one(".price")
                qty_element = variant_row.select_one(".variant-qty")

                if not all([description_element, price_element, qty_element]):
                    continue
