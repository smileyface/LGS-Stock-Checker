...
class UserDBSchema(BaseModel):
    model_config = ConfigDict(from_attributes=True)  # Crucial for converting ORM User objects

    username: str = Field(..., description="The user's unique username.")
    password_hash: str = Field(..., description="The hashed password for authentication.")
...
class UserPublicSchema(BaseModel):
    model_config = ConfigDict(from_attributes=True)  # <--- THIS IS CRUCIAL FOR ORM CONVERSION
